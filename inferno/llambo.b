...
infer(req: ref InferenceRequest): ref InferenceResponse
{
	...
	# Try to use FFI module if available
	response := infer_ffi(req);
	if (response != nil)
		return response;
	
	# Fallback to pure Limbo implementation
	# Tokenize input
	tokens := tokenize(req.ctx, req.prompt);
	
	# Simple echo response for demonstration
	response = ref InferenceResponse;
	response.text = req.prompt + " [generated by llambo inference engine - pure Limbo mode]";
...
# FFI-based inference (uses C module if available)
infer_ffi(req: ref InferenceRequest): ref InferenceResponse
{
	# Try to load FFI module
	llambo_c := load Llambo_c "$Llambo_c";
	if (llambo_c == nil)
		return nil;  # FFI not available, use fallback
	
	# Load model via FFI (cached)
	model := req.ctx.model;
	if (model == nil || model.path == nil)
		return nil;
	
	# Load model through C FFI
	c_model_id := llambo_c->load_model(model.path, model.params.use_mmap, model.params.n_gpu_layers);
	if (c_model_id < 0) {
		print("llambo: FFI model load failed, falling back to pure Limbo\n");
		return nil;
	}
	
	# Run inference via FFI
	result_text := llambo_c->infer(c_model_id, req.prompt, req.max_tokens, req.temperature);
	
	# Build response
	response := ref InferenceResponse;
	response.text = result_text;
	response.tokens = tokenize(req.ctx, result_text);
	response.token_count = len response.tokens;
	response.completion_time = sys->millisec() - sys->millisec();  # Timing handled in C
	
	# Free C model
	llambo_c->free_model(c_model_id);
	
	return response;
}

# Include FFI module declaration
Llambo_c: module
{
	PATH: con "$Llambo_c";
